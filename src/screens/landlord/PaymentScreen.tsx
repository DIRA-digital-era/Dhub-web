...
import { Payment, clearInitiateState, fetchPayments, initiateCollection, upsertPayment } from '../../store/paymentsSlice';
import type { RootState } from '../../store/store';
import { LandlordTabRouteProp } from '../../types';
import { supabase } from '../../utils/supabaseClient';

const RECEIVER_PHONE = '+237600000000';
...
const PaymentsScreen: React.FC = () => {
  ...
  const boostParams = route.params;
  ...
  // Prefill if coming from BoostScreen
  useEffect(() => {
    if (boostParams) {
      setActiveTab('send');
  ...
  const handleSendPayment = () => {
    ...
    dispatch(initiateCollection({
      payerPhone: '651098669',        // TODO: replace with auth state user phone
      amount: String(numAmount),
      reason: 'landlord_subscription',
      client: {
        name: 'Dhub',
        id: `col-${Date.now()}`,
        payer_id: '1966e6b2-bdd8-4e85-a1be-59571fd798ba',
      ...
      {activeTab === 'history' ? (
        ...
        <ScrollView style={styles.sendContainer}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              style={[styles.input, styles.textArea, boostParams && styles.disabledInput]}
              value={boostParams ? 'Boost Listing' : description}
              onChangeText={setDescription}
              multiline
              numberOfLines={3}
              editable={!boostParams && !initiating}
            />
          </View>
