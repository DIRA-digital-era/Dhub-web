...
type BookingWithListing = {
  ...
  rent_payment_status?: string; // GLM: Added to support enhanced tab filtering logic
  caution_status?: string;     // GLM: Added to filter out refund-queued bookings from pending
  created_at?: string;
};
...
export default function ViewBookingsScreen() {
  ...
  const fetchBookingsFromServer = useCallback(async (showLoader = true) => {
    ...
    try {
      ...
      if (error) throw error;
            ...
            // GLM: Added rent_payment_status and caution_status to the mapped object
      const formatted = data.map((b: any) => ({
        id: b.id,
        listing_id: b.listing_id,
        listing_title: b.listings?.title || t('common.unknown'),
        ...
        rent_payment_status: b.rent_payment_status,
        caution_status: b.caution_status,
      ...
      setError(t('bookings.cache_notice'));
        ...
        <View style={styles.cardContent}>
            ...
            <View style={styles.titleContainer}>
              ...
              {isActive && (
                <View style={styles.activeBadge}>
                  ...
                  <Text style={styles.activeBadgeText}>{t('bookings.active')}</Text>
                </View>
              )}
              {isUpcoming && (
                <View style={styles.upcomingBadge}>
                  <Ionicons name="calendar" size={12} color={COLORS.white} />
                  <Text style={styles.upcomingBadgeText}>{t('bookings.upcoming')}</Text>
                </View>
              )}
            </View>
          ...
          <View style={styles.dateContainer}>
            <View style={styles.dateItem}>
              <Text style={styles.dateLabel}>{t('bookings.from')}</Text>
            ...
            <View style={styles.dateItem}>
              <Text style={styles.dateLabel}>{t('bookings.to')}</Text>
              <Text style={styles.dateValue}>
          ...
          <View style={styles.footer}>
            <View style={styles.badgeContainer}>
              <View style={[styles.badge, { backgroundColor: statusColors[item.status] }]}>
                ...
                <Text style={styles.badgeText}>{t(`bookings.${item.status}`)}</Text>
              </View>
              <View style={[styles.badge, { backgroundColor: paymentColors[item.payment_status] }]}>
                <Ionicons 
                  ...
                  color={COLORS.white} 
                />
                <Text style={styles.badgeText}>{t(`bookings.${item.payment_status === 'completed' ? 'paid' : item.payment_status}`)}</Text>
              </View>
            </View>

            <View style={styles.priceContainer}>
              <Text style={styles.amount}>
                {t('listing.fcfa')} {Number(item.amount).toLocaleString(t('common.date_locale'))}
              </Text>
              <Text style={[styles.daysLeft, { color: getDaysLeftColor(daysLeft) }]}>
                {daysLeft >= 0 ? t('bookings.days_left', { count: daysLeft }) : t('bookings.ended')}
              </Text>
            </View>
          </View>
        </View>
  ...
  const renderHeader = () => (
    ...
    // GLM: Updated filter logic to properly bucket bookings based on rent_payment_status
    return bookings.filter(b => {
      const endDate = new Date(b.end_date);
      const isPastDate = endDate < today;
      const isCancelled = b.status === 'cancelled';
      const isCompleted = b.status === 'completed' || isPastDate;
      const isPending = b.status === 'pending' || (b.status === 'confirmed' && b.rent_payment_status === 'pending' && !isCancelled);
      const isRentComplete = b.rent_payment_status === 'completed';
      
      // Active: Lease is active and rent is fully paid, or it's completed but not past date yet
      if (activeTab === 'active') {
        return (b.status === 'confirmed' && (isRentComplete || b.payment_status === 'completed')) && !isPastDate && !isCancelled;
      }
      // Pending: Awaiting landlord approval, or approved/initial fee paid but rent completion pending
      if (activeTab === 'pending') {
        return isPending;
      }
      // Past: Cancelled, completed, or past end date
      if (activeTab === 'past') {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text style={styles.exploreButtonText}>{t('bookings.explore')}</Text>
  ...
  if (error && bookings.length === 0) {
    return <NetworkDisconnectedScreen onRefresh={onRefresh} refreshing={refreshing} />;
  }
